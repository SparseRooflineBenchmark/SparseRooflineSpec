...
The time to transfer input/output data to and from the input file should not be measured.  Timing should begin with inputs resident in memory with the specified binary format (see Section~\ref{sec:formats}). The timing should end after the output is resident in memory with the specified binary format. This benchmark may apply to a diversity of architectures with different memory hierarchies, from distributed memories to special purpose hardware accelerators. The only requirement is that the outputs are resident in the same memory as the inputs, and that only one copy of the input is used. 
...
\section{Correctness and Validation}
    ...
    \subsection{Allowed Optimizations}
        Reference implementations are provided for all kernels in the benchmark suite, computing the operations with some arbitrary associativity for $\sum$. Optimizations which re-associate associative operators are always allowed. Additional optimizations are allowed, but the category of optimizations used must be declared from the following options, which build on each other in terms of permissiveness. (In this categorization, floating point numbers are treated as real numbers):
        \begin{enumerate}
            \item[\texttt{Semigroup}] Optimizations of operators using only associativity. This is the default category and we expect most optimizations fall under this category. This is the category of optimizations allowed under the LINPACK benchmark.
            \item[\texttt{Semiring}] Optimizations using properties derived from operators forming semirings. This category allows optimizations such as Strassen's algorithm.
            \item[\texttt{Field}] Optimizations using properties derived from operators forming rings or fields. Most mathematically equivalent optimizations are allowed here.
        \end{enumerate}

    \subsection{Floating-Point Accuracy}
        Although the previous section treats floating point numbers as real numbers, in practice they behave differently. We expect floating point numbers to handle exceptions exactly matching the reference, and to conform to user-declared relative error bounds, treating the reference implementation as a ground truth.

        \subsection{Exception Handling}
            Although sparse matrices may be entirely zero, we distinguish between explicit (stored) zeros and implicit (not stored) zeros. Exceptional values like \texttt{NaN}s and \texttt{Inf}s are not expected to interact with implicit zeros. In particular, we assume
            \begin{align*}
                \texttt{NaN} * 0 (\text{explicit}) &= \texttt{NaN} \\
                \texttt{NaN} * 0 (\text{implicit}) &= 0 (\text{implicit}) \\
                \texttt{Inf} * 0 (\text{explicit}) &= \texttt{NaN} \\
                \texttt{Inf} * 0 (\text{implicit}) &= 0 (\text{implicit})
            \end{align*}
            
        \subsection{Error Bounds}
            Implementations involving floating point numbers must declare a relative error bound on \eqref{eq:einsum} of the following form, where $p$ and $\epsilon$ are declared parameters:

            \[
                \forall_{i...} \frac{A\_ref_{i...} - A\_res_{i...}}{A\_ref_{i...}} \leq \frac{n^p \epsilon \sum |B_{j...} \cdot C_{k...} \cdot ...|}{A\_ref_{i...}}
            \]

            If only \texttt{Semigroup} level optimizations are used, then the default values $p = 1$ and $\epsilon = 2^{-53}$ will hold.

            This equation is inspired by the standard error bound for recursive summation, taken from \cite{higham}
...
For Spmv, we compare to the naive implemenation and take the L2-norm of the residual. 
